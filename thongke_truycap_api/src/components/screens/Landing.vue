<template>
  <div class="form-chitiet">
    <div >
      <v-layout v-if="itemsReports && itemsReports.length > 0" row wrap class="filter_menu mt-2">
      ...
      <v-layout row wrap class="my-2" v-if="!hiddenAside && itemsReports && itemsReports.length > 0">
        <v-flex xs12>
          <v-btn dark color="blue darken-3" class="mx-0 my-0" v-on:click.native="doCreateReport(false)">
          ...
          <v-btn dark color="blue darken-3" class="my-0" v-on:click.native="doCreateReport(true, 'word')">
          ...
          >
            <template slot="items" slot-scope="props">
              <tr v-bind:class="{'active': props.index%2==1}" style="cursor: pointer;" @click="viewDetail(props.item)">
                ...
                <td class="text-xs-left py-2">
                  <div>
                    <span>{{props.item.hasOwnProperty('apiName') ?  props.item.apiName : ''}}</span>
                  </div>
                </td>
                <td class="text-xs-left py-2">
                  <div>
                    <span>
                      <span>{{props.item.hasOwnProperty('apiDescription') ?  props.item.apiDescription : ''}}</span>
                    </span>
                  </div>
                </td>
                <td class="text-xs-center py-2">
                  <div>
                    <span>
                      <span>{{props.item.totalAccessSuc}}</span>
                    </span>
                  </div>
                </td>
                <td class="text-xs-center py-2">
                  <div>
                    <span>
                      <span>{{props.item.totalAccessFal}}</span>
                    </span>
                  </div>
                </td>
                <td class="text-xs-center py-2">
                  <div>
                    <span>
                      <span>{{props.item.totalAccess}}</span>
                    </span>
                  </div>
                </td>
              </tr>
...
export default {
  props: ['index', 'dataImport', 'fromDate', 'toDate', 'month', 'quarter', 'year'],
  components: {
  ...
  methods: {
    ...
    doDynamicReport () {
      ...
      let timeTileReplace = ''
      if (vm.fromDate) {
        timeTileReplace = 'Từ ngày ' + vm.fromDate + ' đến ngày ' + vm.toDate
      }
      if (vm.month) {
        timeTileReplace = 'Tháng ' + vm.month + ' năm ' + vm.year
      }
      if (vm.quarter) {
        timeTileReplace = 'Quý ' + vm.quarter + ' năm ' + vm.year
      }
      if (vm.year && !vm.month && !vm.quarter) {
        timeTileReplace = 'Năm ' + vm.year
      }
      docDString = docDString.replace(/\[\$timeTitle\$\]/g, timeTileReplace)
      let widthsConfig = []
