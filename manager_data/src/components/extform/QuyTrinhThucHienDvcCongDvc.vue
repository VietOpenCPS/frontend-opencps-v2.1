...
export default {
  ...
  methods: {
    ...
    openserviceProcessOptionList () {
      ...
      vm.$router.push({
        path: '/table/opencps_processoption_dvc/editor/0?serviceConfigId=' + vm.serviceConfigId + '&serviceConfigName=' + vm.serviceConfigName + '&col=serviceConfigId&pk=' + vm.serviceConfigId
      })
    },
    ...
    editServiceProcessOption (item) {
      ...
      let newQuery = current.query
      vm.$router.push({
        path: '/table/opencps_processoption_dvc/editor/' + item.processOptionId + '?serviceConfigId=' + vm.serviceConfigId + '&serviceConfigName=' + vm.serviceConfigName,
      })
    },
    deleteServiceProcessOption (item) {
      ...
      if (x) {
        ...
        dataPost.append('url', '/serviceconfigs/' + vm.serviceConfigId + '/processes/' + item.processOptionId)
        dataPost.append('data', JSON.stringify(params))
        axios.post('/o/rest/v2/proxy', dataPost, param).then(function (response) {
