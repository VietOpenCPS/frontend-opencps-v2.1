  ...
  <div style="padding: 0px 25px; max-width: 1500px; margin: 0px auto" class="mt-0 pb-3" id="top-header">
    ...
    <v-card class="px-3 py-3" style="width: 100%; background: #ffffff;border-radius: 12px;box-shadow: 0 6px 10px -4px rgb(0 0 0 / 15%)">
      ...
      <div class="my-2">
        ...
        <span style="font-size: 16px;">Vui lòng tải mẫu import số hóa mới (Cập nhật ngày 5/8/2022)</span>
      </div>
      <div class="my-2">
        <span style="color: red">(*) </span>
        <span style="font-size: 16px;">Cài đặt plugin kết nối thư mục tài liệu cần ký số </span>
        <a target="_blank" href="https://chrome.google.com/webstore/detail/web-server-for-chrome/ofhbbkphhbklhfoeikjpcbhemlocgigb" 
        style="color: #0072bc;font-size: 16px;margin-left: 10px;text-decoration: underline;">web-server-for-chrome</a>
      </div>
      <div class="my-2">
        <span style="color: red">(*) </span>
        <span style="font-size: 16px;">Đường dẫn thư mục cần ký số: </span>
        <v-text-field
          style="display: inline-block; width: 170px; margin-left: 10px;font-size: 16px;"
          v-model="webServerUrl"
          single-line
        ></v-text-field>
      </div>
      ...
      <v-btn color="primary" small class="mx-2 white--text" :loading="processingSigning" :disabled="processingSigning" @click.stop="uploadDocumentSign">
        <v-icon size="20" style="color: #fff !important">
          upload_file
        </v-icon> &nbsp;
      ...
      <v-btn color="primary" small class="mx-0 white--text ml-2 kyso-btn" :loading="processingSigning" :disabled="processingSigning" @click.stop="signDocument">
        <v-icon size="20" style="color: #fff !important">
      ...
      >
        ...
        <template slot="items" slot-scope="props">
          <tr v-bind:class="{'active': props.index%2==1}" style="cursor: pointer;">
            ...
            <td class="text-xs-left py-2" style="height:36px; min-width:150px; max-width: 350px">
              <content-placeholders v-if="loadingTable">
                <content-placeholders-text :lines="1" />
              </content-placeholders>
              <div v-else>
    ...
    <input type="file" id="fileKySoUpload" accept=".pdf, .PDF" multiple @input="uploadFileKySo($event)" style="display:none">
  </div>
</template>
    ...
    methods: {
      ...
      uploadDocumentSign () {
        ...
        if (vm.selectedDocument && vm.selectedDocument.length) {
          vm.$confirm(
            {
              message: 'Tổng số ' + vm.selectedDocument.length + ' giấy tờ sẽ được số hóa. Vui lòng chọn tài liệu tải lên.',
              button: {
                no: 'Thoát',
                yes: 'Đồng ý'
              },
      ...
      pickFileUploadGiayTo () {
        document.getElementById('fileKySoUpload').value = ''
        document.getElementById('fileKySoUpload').click()
      },
      uploadFileKySo () {
        let vm = this
        let files = $('#fileKySoUpload')[0].files
        let countFiles = files.length
        let arrAction = []
        if (files.length > 0) {
          for (let index = 0; index < files.length; index++) {
            let mappingExcelData = vm.selectedDocument.find(function (item) {
              return item['tenFile'] == files[index]['name']
            })
            if (!mappingExcelData) {
              vm.$confirm(
                {
                  message: 'Không tìm thấy thông tin số hóa cho giấy tờ "' + files[index]['name'] + '". Vui lòng kiểm tra lại',
                  button: {
                    no: 'Thoát',
                    yes: 'Đồng ý'
                  },
          ...
          vm.processingSigning = true
          for (let index = 0; index < countFiles; index++) {
            arrAction.push(vm.$store.dispatch('uploadFileKySo', files[index]))
          }
          if (arrAction.length) {
            Promise.all(arrAction).then(results => {
              console.log('uploadFileKySo', results)
              let dataFileUpload = results.filter(function (item) {
                return item.Status == true && item.FileName
              })
              if (dataFileUpload.length) {
                let dossierFiles = []
                for (let index = 0; index < dataFileUpload.length; index++) {
                  let excelMapping = ''
                  try {
                    excelMapping = vm.selectedDocument.find(function (item) {
                      return item.tenFile == dataFileUpload[index]['fileNameInput']
                    })
                  } catch (error) {
                  }
                  dataFileUpload[index]['excelDataId'] = excelMapping ? excelMapping['excelDataId'] : ''
                }
                dataFileUpload.forEach(element => {
                  if (element.excelDataId) {
                    let signed = JSON.parse(element.FileServer)
                    let fileEntryId = signed['fileEntryId']
                    let dataItem = {
                      fileEntryIdStr: fileEntryId,
                      excelDataIdStr: element.excelDataId
                    }
                ...
                vm.$store.dispatch('updateFileKySoVgca', dataUpdateFile).then(res => {
                  vm.processingSigning = false
                  let errCount = vm.selectedDocument.length - dataFileUpload.length
                  vm.$confirm(
                    {
                      message: 'Tổng số: ' + vm.selectedDocument.length + ', Thành công: ' + dataFileUpload.length + ', Thất bại: ' + errCount,
                      button: {
                        no: 'Thoát',
                        yes: 'Đồng ý'
                      },
                ...
                vm.$confirm(
                  {
                    message: 'Tổng số: ' + vm.selectedDocument.length + ', Thành công: 0, Thất bại: ' + vm.selectedDocument.length,
                    button: {
                      no: 'Thoát',
                      yes: 'Đồng ý'
                    },
        ...
        vm.$store.dispatch('getApplicantDocument', filter).then(function (result) {
          if (result.hasOwnProperty('data')) {
            vm.documentApplicantList = Array.from(result.data, function (e) {
              return Object.assign(e, {tenFileFull: vm.webServerUrl + '/' + e.tenFile})
            })
          } else {
