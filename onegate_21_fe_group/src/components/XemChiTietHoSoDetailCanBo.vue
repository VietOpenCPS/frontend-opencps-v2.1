  ...
  methods: {
    ...
    btnActionEvent (item, index) {
      ...
      if (String(item.form) === 'UPDATE') {
        vm.$router.push({
          path: vm.pathLanding + vm.index + '/ho-so/' + vm.thongTinChiTietHoSo.dossierId + '/' + vm.itemAction.form,
          query: vm.$router.history.current.query
        })
      } else if (String(item.form) === 'ADD') {
        vm.$router.push({
          path: vm.pathLanding + vm.index + '/bo-sung-ho-so/' + vm.thongTinChiTietHoSo.dossierId,
          query: vm.$router.history.current.query
        })
              ...
              if (filter['toUsers'] && filter['toUsers'].filter(function (item) {
                ...
                vm.$store.dispatch('processDossierRouter', filter).then(function (result) {
                  ...
                  if (vm.thongTinChiTietHoSo.dossierStatus === 'new' && vm.originality === 1) {
                    vm.$router.push(vm.pathLanding + vm.index + '/nop-thanh-cong/' + vm.thongTinChiTietHoSo.dossierId)
                  }
                  vm.checkInput = 0
