    ...
    <v-dialog v-model="dialog_createDocument" scrollable persistent max-width="900px">
      <v-card>
        ...
        <v-card-actions class="mx-3">
          <v-spacer></v-spacer>
          <v-btn class="mr-2" color="primary" @click.native="dialog_createDocument = false">
            <v-icon>clear</v-icon> &nbsp;
            Thoát
          </v-btn>
          <v-btn v-if="typeCreate === 'create'" class="mr-0" color="primary" @click.native="createDocument">
            <v-icon>save</v-icon> &nbsp;
            <span>Thêm tài liệu</span> 
          </v-btn>
          <v-btn v-else class="mr-0" color="primary" @click.native="updateDocument">
            <v-icon>save</v-icon> &nbsp;
...
export default {
  ...
  methods: {
    ...
    viewDocument (item) {
      ...
      // ) {
      //   vm.dialogPDF = true
      //   document.getElementById('dialogPDFPreview').src = url
      // } else {
        window.location.assign(url)
      // }
    },
