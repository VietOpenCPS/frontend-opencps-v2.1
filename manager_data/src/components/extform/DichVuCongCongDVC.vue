                ...
                <tr v-bind:class="{'active': props.index%2==1}" style="cursor: pointer;">
                    ...
                    <td class="text-xs-center px-2 py-2" style="width:290px;height:36px;border-right: 1px solid #dedede">
                        ...
                        <div v-else>
                            ...
                            >
                              <v-icon :size="18" style="color: white !important;">settings</v-icon> &nbsp; Xác lập quy trình
                            </v-btn>
                        </div>
                    </td>
                </tr>
...
export default {
  ...
  methods: {
    ...
    getDomains (agencyCode) {
      ...
      vm.domainList = []
      vm.$store.dispatch('getDomainDvc', data).then(
        res => {
          vm.domainList = res
        }
      ).catch(()=>{
    ...
    getServiceInfo (agencyCode, domainCode) {
      ...
      vm.serviceInfoList = []
      vm.$store.dispatch('getServiceInfoDvc', data).then(function(res) {
    ...
    openCreateServiceConfig () {
      ...
      if (vm.govAgencyFilter) {
        if (vm.isDvc && !vm.boNganh) {
          vm.$router.push({
            path: '/table/opencps_serviceconfig_dvc/editor/0?administration=' + vm.capCoQuanThucHien + '&govAgencyCode=' + vm.govAgencyFilter + '&domainCode=' + vm.domainFilter + '&serviceCode=' + vm.serviceFilter + '&level=' + vm.levelFilter,
          })
        } else {
          vm.$router.push({
            path: '/table/opencps_serviceconfig_dvc/editor/0?govAgencyCode=' + vm.govAgencyFilter + '&domainCode=' + vm.domainFilter + '&serviceCode=' + vm.serviceFilter + '&level=' + vm.levelFilter,
          })
        }
      } else {
        if (vm.isDvc) {
    ...
    getServiceConfigList (reset) {
      ...
      dataPost.append('url', '/serviceconfigs')
    ...
    editServiceConfig (item) {
      ...
      let newQuery = current.query
      vm.$router.push({
        path: '/table/opencps_serviceconfig_dvc/editor/' + item.serviceConfigId,
        query: newQuery
      })
    },
    deleteServiceConfig (item) {
      ...
      if (x) {
        ...
        dataPost.append('url', '/serviceconfigs/' + item['serviceConfigId'])
        dataPost.append('data', JSON.stringify(params))
        axios.post('/o/rest/v2/proxy', dataPost, param).then(function (response) {
    ...
    updateServiceProcessOption (item) {
      ...
      let newQuery = current.query
      vm.$router.push({
        path: '/table/opencps_processoption_dvc?serviceConfigId=' + item.serviceConfigId + '&serviceConfigName=' + item.serviceName
      })
    },
    parsedate (date) {
