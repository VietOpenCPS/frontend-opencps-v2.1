            ...
            <div slot="header" @click="stateView = false" style="background-color:#fff">
              <div style="align-items: center;background: #fff; padding-left: 25px;" :style="{width: checkStyle(item)}">
                ...
                <div style="display: flex">
                  ...
                  required
                  @change="item.fileComment=String(item.fileComment).trim()"
              ...
              <v-card-text style="background-color: rgba(244, 247, 213, 0.19)">
                <v-layout wrap>
                  <v-flex xs12 class="text-xs-right" v-if="!stateView">
                    ...
                    <div :id="'formAlpaca' + item.partNo + id" :class='{"no_acction__event": onlyView}' v-if="item['editForm']">
                    </div>
                    <!--  -->
            ...
            <v-card-text class="pb-0 pt-4">
              <v-layout wrap>
                <v-flex xs12 class="px-2 pb-3 fix__label">
                  ...
                  required
                  @change="otherDossierTemplate=String(otherDossierTemplate).trim()"
                  ></v-text-field>
                </v-flex>
              </v-layout>
            </v-card-text>
...
export default {
  ...
  methods: {
    ...
    getFilesGroupDossier () {
      let vm = this
      return vm.dossierFilesItems
    },
