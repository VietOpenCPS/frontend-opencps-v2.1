<template>
  <div class="taotailieuketqua" style="background-color: white;width:100%" v-if="active">
          ...
          <div class="form_alpaca" style="position: relative;" v-for="(item, index) in createFiles" v-bind:key="item.partNo + 'cr'">
              ...
              <v-expansion-panel-content hide-actions>
                <div slot="header" @click="stateView = false" style="background-color:#fff">
                  <div style="align-items: center;background: #fff; padding-left: 25px;" :style="{width: checkStyle(item)}">
                    <div class="mr-2" @click="loadAlpcaFormClick(item, index)" style="min-width: 20px; display: flex;">
            ...
            <div class="absolute__btn group__thanh_phan pr-3 mr-1">
              ...
              <v-layout row wrap v-else>
                <v-flex :style="(esignType === 'plugin' || esignType === 'pluginAndHSM') ? 'width: 170px' : 'width: 150px'">
                  ...
                  <v-tooltip top v-if="progressUploadPart + id !== item.partNo + id && item.eForm">
                    <v-btn slot="activator" icon class="mx-0 my-0" @click="loadAlpcaFormClick(item, index)">
                      <v-badge>
    ...
    <!--  -->
    <v-dialog v-model="dialog_formscript_tphs" scrollable persistent fullscreen>
      <v-card>
        <v-toolbar dark color="primary">
          <v-toolbar-title>
        ...
        <v-card-text class="py-1">
          <form-thanh-phan-ho-so ref="formThanhPhanHoSo" :dataInput="formDataTphs" :mauNhap="mauNhapForm" :readonly="onlyView"
            :thongtinhoso="detailDossier"
          >
          </form-thanh-phan-ho-so>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn class="mr-2" color="primary" @click.native="dialog_formscript_tphs = false"
...
<script>
  ...
  export default {
    ...
    components: {
      ...
      'form-thanh-phan-ho-so': FormThanhPhanHoSo
    },
    data: () => ({
      docxTemplateUrl: '',
      mauNhapForm: '',
    ...
    methods: {
      submitFormTphs() {
        ...
        if (validate) {
          ...
          if (fileFind) {
            ...
            vm.loadingSubmitFormTphs = true
            vm.$store.dispatch('putFormNghiepVu', fileFind).then(resData => {
              setTimeout(function () {
                vm.loadingSubmitFormTphs = false
                toastr.clear()
                toastr.success('Thực hiện thành công')
                vm.dialog_formscript_tphs = false
                vm.createFiles[vm.indexThanhPhanFormUpdate].daKhai = true
            ...
            vm.thanhPhanFormUpdate['dossierId'] = vm.detailDossier.dossierId
      ...
      exportDoc (part, index) {
        let vm = this
        vm.$store.dispatch('loadFormScriptKQ', part).then(resScript => {
          /* eslint-disable */
          let eformScript = ''
          try {
      ...
      loadAlpcaFormClick (data, index) {
        ...
        vm.createFiles.forEach(val => {
          if (val.eForm && data.partNo === val.partNo) {
            val['templateNo'] = vm.detailDossier.dossierTemplateNo
            vm.$store.dispatch('loadFormScriptKQ', val).then(resScript => {
              /* eslint-disable */
              let eformScript = ''
              try {
                eformScript = eval('(' + resScript + ')')
              ...
              if (eformScript && eformScript.hasOwnProperty('eformDHQG') && eformScript.eformDHQG) {
                ...
                vm.$store.dispatch('getListDossierFiles', vm.detailDossier.dossierId).then(result => {
