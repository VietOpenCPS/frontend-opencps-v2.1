  ...
  <div style="width: 1300px; margin: 0 auto">
    <v-flex class="text-right">
      <div class="d-inline-block px-1 mr-2" style="border: 1px solid #dedede;border-radius: 4px;background: #fff;">
        <v-btn @click="toNativeViewType(true)" :flat="chartView ? false : true" depressed small color="#903938"
        class="mr-2" :class="chartView ? 'white--text' : ''" style="height: 30px;">Đơn vị huyện</v-btn>
        <v-btn @click="toNativeViewType(false)" :flat="chartView ? true : false" depressed small color="#903938" 
        :class="chartView ? '' : 'white--text'" style="height: 30px;">Đơn vị cấp xã</v-btn>
      </div>
    </v-flex>
        ...
        <div class="pa-2 v-sheet theme--light" style="border: 1px solid #dedede;">
          ...
          <v-layout wrap>
            <v-flex xs12 class="my-2 report__table" style="overflow: hidden;">
              <v-flex xs12 class="text-center text-bold my-3" v-if="chartView">
                TỔNG HỢP TÌNH HÌNH GIẢI QUYẾT HỒ SƠ <span style="text-transform: uppercase;">{{govAgencyInfo['govAgencyName']}}</span> <br/>
                <span v-if="String(monthSelected) !== '0'">Tháng {{monthSelected}}</span> Năm {{yearSelected2}}
              </v-flex>
              <v-flex xs12 class="text-center text-bold my-3" v-else>
                TỔNG HỢP TÌNH HÌNH GIẢI QUYẾT HỒ SƠ CẤP XÃ <span style="text-transform: uppercase;">{{govAgencyInfo['govAgencyName']}}</span> <br/>
                <span v-if="String(monthSelected) !== '0'">Tháng {{monthSelected}}</span> Năm {{yearSelected2}}
              </v-flex>
              <table class="my-2" hide-default-footer>
                ...
                <tbody v-if="!loadingTable && danhSachThongKe.length > 0">
                  ...
                  <tr v-for="(item,index) in danhSachThongKe" :key="index">
                    ...
                    <td align="left" class="px-2" style="padding: 8px 10px;">{{chartView ? item.domainName : item.govAgencyName}}</td>
...
export default {
  ...
  created() {
    let vm = this
    vm.$nextTick(() => {
      console.log('govAgencyInfo', vm.govAgencyInfo)
  ...
  methods: {
    ...
    getStatisticsYear() {
      ...
      let config = {
        ...
        headers: {
          groupId: vm.chartView ? window.themeDisplay.getScopeGroupId() : vm.govAgencyInfo['groupIdCapXa'],
    ...
    viewDossierList (itemRecord, statusCount) {
      ...
      if (vm.chartView === false) {
        vm.groupIdAgency = vm.govAgencyInfo['groupIdCapXa']
