      ...
      <div class="grid-list mt-3">
          <v-layout wrap>
              ...
              <v-flex xs12   class="text-xs-right">
                <v-btn :loading="loading" :disabled="loading" color="blue darken-3" class="white--text" @click="postSyncStatistics()">Gửi đi</v-btn>
              </v-flex>
          </v-layout>
      </div>
    </v-card>
...
export default {
  ...
  data: () => ({
    ...
    months: [{name: 'Tháng 1', value: 1}, {name: 'Tháng 2', value: 2}, {name: 'Tháng 3', value: 3}, {name: 'Tháng 4', value: 4}, {name: 'Tháng 5', value: 5}, {name: 'Tháng 6', value: 6}, {name: 'Tháng 7', value: 7}, {name: 'Tháng 8', value: 8}, {name: 'Tháng 9', value: 9}, {name: 'Tháng 10', value: 10}, {name: 'Tháng 11', value: 11}, {name: 'Tháng 12', value: 12}],
    years: [],
    ...
    fields: [
      ...
      {
        label: 'Tổng đã xử lý đúng hạn',
        value: 'TongXuLyDungHan',
        class: 'xs12 md2',
        model: ''
      },
      {
        label: 'Phần trăm đã xử lý đúng hạn',
        value: 'PhanTramXuLyDungHan',
        class: 'xs12 md2',
        model: ''
      },
      {
        label: 'Tổng đã xử lý trễ hạn',
        value: 'TongXuLyTreHan',
        class: 'xs12 md2',
        model: ''
      },
      {
        label: 'Phần trăm đã xử lý trễ hạn',
      ...
      {
        label: 'Tổng chưa xử lý quá hạn',
        value: 'TongChuaXuLyTreHan',
        class: 'xs12 md2',
        model: ''
      },
      {
        label: 'Phần trăm chưa xử lý quá hạn',
    ...
    vm.$nextTick(function () {
        ...
        vm.$store.dispatch('getStatistics',filter).then(function (result) {
          ...
          if(result.data && result.data.length) {
            ...
            vm.dataPOST.data[0]['TongXuLyDungHan'] = String(result.data[0].betimesCount + result.data[0].ontimeCount)
            vm.dataPOST.data[0]['PhanTramXuLyDungHan'] = String((((result.data[0].betimesCount + result.data[0].ontimeCount) / result.data[0].releaseCount)*100).toFixed(2))
            vm.dataPOST.data[0]['TongXuLyTreHan'] = String(result.data[0].overtimeCount)
            vm.dataPOST.data[0]['PhanTramXuLyTreHan'] = String(((result.data[0].overtimeCount/result.data[0].releaseCount)*100).toFixed(2))
            vm.dataPOST.data[0]['TongChuaXuLy'] = String(result.data[0].processingCount)
            vm.dataPOST.data[0]['TongChuaXuLyTrongHan'] = String(result.data[0].undueCount)
            vm.dataPOST.data[0]['PhanTramChuaXuLyTrongHan'] = String(((result.data[0].undueCount/result.data[0].processingCount)*100).toFixed(2))
            vm.dataPOST.data[0]['TongChuaXuLyTreHan'] = String(result.data[0].overdueCount)
            vm.dataPOST.data[0]['PhanTramChuaXuLyTreHan'] = String(((result.data[0].overdueCount/result.data[0].processingCount)*100).toFixed(2))
          } else {
