              ...
              if (dateStr !== 'Invalid Date' && String(currentVal).length === 13 && !isNaN(dateStr)) {
                ...
                if (String(key).indexOf('DateExtend') > 0) {
                  ...
                  if (day && month && year && filter.hasOwnProperty('formatDate') && filter.formatDate === 'timestamp') {
                    ...
                    if (key === 'fromDate') {
                      timeStampDate = (new Date(`${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}T00:00`)).getTime()
                    }
                    if (key === 'toDate') {
    ...
    getAgencyReportListsOld ({state}, filter) {
      return new Promise((resolve, reject) => {
        store.dispatch('loadInitResource').then(function () {
          ...
          let requestURL = filter['api']
          // 
          if (filter['exportVoting']) {
            param.params['isExcel'] = true
            let config = {
              method: 'get',
              url: filter['api'],
      ...
      return new Promise((resolve, reject) => {
        store.dispatch('loadInitResource').then(function () {
          axios({
            ...
            let serializable = response.data
            if (filter['download']) {
              FileSaver.saveAs(serializable, new Date().getTime() + '.xlsx')
            } else {
    ...
    loadDataSource ({commit, state}, filter) {
      return new Promise((resolve, reject) => {
        ...
        store.dispatch('loadInitResource').then(function () {
          ...
          } else if (filter.hasOwnProperty('typeSource') || (filter.hasOwnProperty('groupId') && (filter.groupId === '' || String(filter.groupId) === '0'))) {
            // lấy qua proxy
            let getAllUrlParams = function(arr) {
              let obj = {}
              for (var i = 0; i < arr.length; i++) {
