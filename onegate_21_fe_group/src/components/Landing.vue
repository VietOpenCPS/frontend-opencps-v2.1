<template>
  <div>
    <div class="row-header no__hidden_class">
      ...
      <div v-if="trangThaiHoSoList !== null" class="layout row wrap header_tools row-blue">
    ...
    <!--  -->
    <div v-if="trangThaiHoSoList !== null" class="menu_header_list py-2" :class='{"no__border__bottom": btnDynamics === null || btnDynamics === undefined || btnDynamics === "undefined" || (btnDynamics !== null && btnDynamics !== undefined && btnDynamics !== "undefined" && btnDynamics.length === 0)}'>
    ...
    <div class="text-xs-right layout wrap" style="position: relative;" v-if="trangThaiHoSoList !== null">
  ...
  methods: {
    ...
    btnActionEvent (dossierItem, item, index, isGroup) {
      ...
      if (String(item.form) === 'NEW' || String(item.form) === 'NEW_GROUP') {
       ...
       vm.$router.push({
          path: vm.pathLanding + vm.index + '/ho-so/' + dossierItem.dossierId + '/' + vm.itemAction.form,
          query: vm.$router.history.current.query
        })
      } else if (String(item.form) === 'ADD') {
       vm.$router.push({
          path: vm.pathLanding + vm.index + '/bo-sung-ho-so/' + dossierItem.dossierId,
          query: vm.$router.history.current.query
        })
      ...
      if (isGroup) {
        ...
        if (vm.selectedDoAction.length === 1) {
          for (let key in vm.selectedDoAction) {
           ...
           vm.$router.push({
              path: vm.pathLanding + vm.index + '/chi-tiet-ho-so/' + actionDossierItem['dossierId'],
              query: {
                activeTab: 'tabs-1',
                btnIndex: null
              }
          ...
          vm.$store.dispatch('loadActionActive', item).then(function () {
            vm.$store.dispatch('loadDossierSelected', vm.selectedDoAction).then(function () {
             ...
             vm.$router.push({
                path: vm.pathLanding + vm.index + '/xu-ly-ho-so',
                query: query
              })
            }).catch(function (){})
          }).catch(function (){})
    ...
    doCopy (dossierItem, item, index, isGroup) {
      ...
      vm.$store.dispatch('doCopy', filter).then(function (result) {
        ...
        vm.indexAction = -1
       vm.$router.push({
          path: vm.pathLanding + vm.index + '/ho-so/' + result.dossierId + '/' + vm.itemAction.form,
          query: vm.$router.history.current.query
        })
      }).catch(reject => {
        vm.loadingAction = false
    ...
    processPullBtnDetailRouter (dossierItem, item, result, index, btnIndex) {
      ...
      if (isPopup) {
        ...
        */
        ...
        // }
       vm.$router.push({
          path: vm.pathLanding + vm.index + '/chi-tiet-ho-so/' + dossierItem['dossierId'],
          query: query
        })
      } else {
        vm.processAction(dossierItem, item, result, index, true)
    ...
    viewDetail (item, indexItem) {
      ...
      if (item.permission) {
        if (item['originality'] === 9) {
          vm.$router.push({
            path: vm.pathLanding+ this.index +'/nhom-ho-so/' + item.dossierId,
            query: vm.$router.history.current.query
          })
        } else {
          vm.$router.push(vm.pathLanding + this.index + '/chi-tiet-ho-so/' + item['dossierId'])
        }
      } else {
