    ...
    <div>
      <div class="row-header no__hidden_class">
        <div class="background-triangle-big">
          <span>DANH SÁCH HỒ SƠ HOÀN THÀNH TRỄ HẠN</span>
        </div>
      </div>
      ...
      <div style="width: 100%;height: 30px;">
        <v-flex xs12 class="right">
          <v-btn color="primary" @click.native="submitDossierStatus()" class="mx-0"
      ...
      <!--  -->
      <v-dialog v-model="dialogUpdateDossier" max-width="700" transition="fade-transition">
        <v-card>
          <v-form ref="formUpdate" v-model="validFormUpdate" lazy-validation>
            <v-toolbar dark color="primary">
              <v-toolbar-title>Lý do trễ hạn</v-toolbar-title>
              <v-spacer></v-spacer>
              <v-btn icon dark @click.native="dialogUpdateDossier = false">
                <v-icon>close</v-icon>
              </v-btn>
            </v-toolbar>
              ...
              <v-layout wrap>
                <v-flex xs12 class="px-2 pb-3">
                  <v-textarea
                    ...
                    :rows="5"
                    box
                    :rules="[v => !!v || 'Vui lòng nhập lý do trễ hạn']"
                    required
                    placeholder="Nhập lý do trễ hạn hồ sơ"
                  ></v-textarea>
                </v-flex>
              </v-layout>
            </v-card-text>
            <v-card-actions class="mx-3">
              <v-spacer></v-spacer>
              <v-btn class="pt-0" color="primary" flat="flat" @click.native="submitUpdateDossier()"
                :loading="loadingActionUpdate"
                :disabled="loadingActionUpdate"
              >
                <v-icon>save</v-icon>&nbsp;
                Cập nhật
                <span slot="loader">Loading...</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitUpdateDossier () {
        let vm = this
        if (vm.$refs.formUpdate.validate()) {
          let x = confirm('Bạn có chắc chắn thực hiện cập nhật trạng thái với ' + vm.dossierSelected.length + ' hồ sơ đã chọn')
          if (x) {
            vm.loadingActionUpdate = true
            let dossierIdsSelect = vm.dossierSelected.map(select => {
              return select.dossierId
            }).join(',')
            let dossierIdsSelectArr = dossierIdsSelect.split(',')
            let params = {
              dossierIds: dossierIdsSelect,
              actionCode: '9669',
              actionNote: vm.actionNote
            }
            vm.$store.dispatch('updateDossierStatus', params).then(function() {
              setTimeout(function() {
