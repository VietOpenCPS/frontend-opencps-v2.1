    ...
    <v-layout row wrap :class="isMobile ? 'pt-0' : 'pt-2'">
      <v-flex xs12 md3>
        ...
        <v-card v-if="!qrscan" flat color="#ffffff" class="px-2 py-2">
          ...
          <v-flex class="py-2 text-xs-center captcha-element" xs12 style="
            align-items: center;
            background: #dedede;
          ...
          <v-flex xs12 class="mb-1">
            <div v-if="forEPayment"><span style="color: red">(*) </span> Nhập mã hồ sơ và mã captcha để thực hiện thanh toán trực tuyến.</div>
            <div v-else><span style="color: red">(*) </span> Nhập mã hồ sơ và mã captcha để thực hiện tra cứu hồ sơ.</div>
          </v-flex>
  ...
  methods: {
    ...
    doSearchDossier () {
      ...
      let filter = {
        ...
        status: vm.status,
        order: true,
        captcha: vm.j_captcha_response
      }
      if (vm.dossierNoKey && vm.j_captcha_response) {
        if (window.themeDisplay.isSignedIn() && !vm.applicantIdNo) {
          filter.applicantIdNo = vm.userInfo.applicantIdNo
        }
        vm.$store.dispatch('loadingDataHoSoCaptcha', filter).then(function (result) {
          vm.loading = false
    ...
    changeDataSearch () {
      ...
      if (vm.dossierNoKey && vm.j_captcha_response) {
        ...
        toastr.error('Vui lòng nhập mã hồ sơ và mã captcha để tra cứu')
      }
    },
    traCuuHoSoVanTai () {
      let vm = this
